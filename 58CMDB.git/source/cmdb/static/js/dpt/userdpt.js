...
$('#add_user').on('click', function(event){
    var dpt_id = $('#dpt_info').attr('value');
    var user_name  =  $('#user_name').val();
    var is_admin = $('#is_admin').is(":checked")?1:0;
    var op = $('#control_type').val();
    $.get('/api/dptuser/adduser/', {'dpt_id':dpt_id, 'user_name':user_name, 'is_admin':is_admin, 'op':op}, function(data){
        if(data.code != 0){
                swal({
                    title: "保存失败",
                    type: "warning",
...
$('.del-btn').on("click", function(event){
    ...
    $.get('/api/dptuser/deluser/',{id:value}, function(data){
         if(data.code != 0){
                swal({
                    title: "删除失败",
                    type: "warning",
